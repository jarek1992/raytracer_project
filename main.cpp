...
int main() {
	...
	point3 v0(0, 0, 0), v1(1, 0, 0), v2(0, 1, 0);
	vec3 n = unit_vector(cross(v1 - v0, v2 - v0));
	auto triangle1 = make_shared<triangle>(v0, v1, v2, n, n, n, triangle_material);
	...
	//6. .obj MODEL LOADER TEST
	auto teapot_model2 = make_shared<model>("assets/models/teapot.obj", emmissive_material, 0.3);
	auto roted_teapot_x = make_shared<rotate_x>(teapot_model2, -90.0);
	auto roted_teapot_y = make_shared<rotate_y>(roted_teapot_x, 30.0);
	world.add(make_shared<translate>(roted_teapot_y, point3(0.0, 0.8, 0.0)));
	...
	//HDRI sky settings
	cam.sky_intesity = 0.75; //intensity multiplier for sky color
