...
int main() {
	...
	//0. .obj MODEL LOADER TEST
	auto debug_mat = make_shared<metal>(color(1.0, 0.85, 0.3), 0.0);
	auto teapot_model2 = make_shared<model>("assets/models/teapot.obj", debug_mat, 0.3);
	auto roted_teapot_x = make_shared<rotate_x>(teapot_model2, -90.0);
	auto roted_teapot_y = make_shared<rotate_y>(roted_teapot_x, 30.0);
	world.add(make_shared<translate>(roted_teapot_y, point3(0.0, 0.9, 0.0)));
